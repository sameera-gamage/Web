...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!hash_equals($_SESSION['csrf'], $token)) {
        ...
        if ($name === '' || ($email === '' && $phone === '')) {
            ...
            $st = db()->prepare('INSERT INTO enquiries (name,email,phone,plot,message) VALUES (?,?,?,?,?)');
            $st->execute([$name, $email, $phone, $plot, $msg]);

            // best-effort email to Sales (works when the host has mail configured)
...
<main>
  <section class="page-hero">
    <div class="wrap">
      ...
      <h1 class="display reveal d1">Have a plot,<br>or looking for one?</h1>
      <p class="lead muted reveal d2" style="max-width:52ch;margin-top:1rem">Tell us where you are in the journey. We answer every message ourselves, from Sales in Mount Lavinia.</p>
    </div>
  </section>
  ...
  <section class="section" style="padding-top:0">
    <div class="wrap">
      <div class="contact-grid">
        <div class="reveal">
          <?php if ($sent): ?>
            <div class="alert alert-ok">Thank you. Your enquiry is with our Sales team and we will be in touch shortly.</div>
          <?php elseif ($error): ?>
          ...
          <form class="form" method="post" action="<?= url('contact.php') ?>">
            ...
            <div style="margin-top:1.4rem"><button class="btn btn-lg" type="submit">Send enquiry</button></div>
          </form>
        </div>
        ...
        <aside class="reveal d1">
          ...
          <div class="map">
            <iframe title="Map to Excello" loading="lazy" referrerpolicy="no-referrer-when-downgrade"
              src="https://maps.google.com/maps?q=<?= rawurlencode(SITE_ADDRESS . ', Sri Lanka') ?>&output=embed"></iframe>
          </div>
        </aside>
      </div>
    </div>
